/**
 * Tool catalog for CougarCalc Phase 2.
 * Only two calculators are built. Unbuilt tools link to their category page
 * so related-tool lists never 404.
 */

import { DELAY_CATEGORY_ONELINER, DELAY_TITLE } from '../site.ts';
...
export type ToolEntry = {
	...
	built: boolean;
	/** Public href. Built tools get a page; others point at the category stub. */
	href: string;
};
...
export function builtAudioTools(): ToolEntry[] {
	return AUDIO_TOOLS.filter((t) => t.built);
}

export function builtGuitarTools(): ToolEntry[] {
	return GUITAR_TOOLS.filter((t) => t.built);
}

export function relatedAudio(currentSlug: string): ToolEntry[] {
	return AUDIO_TOOLS.filter((t) => t.slug !== currentSlug);
}

export function relatedGuitar(currentSlug: string): ToolEntry[] {
	return GUITAR_TOOLS.filter((t) => t.slug !== currentSlug);
}
